...
public class RestClientExecutor {
     ...
     */
    public <T> T postObject(final String url, final Object postBody, final TypeReference<Response<T>> responseTypeReference) throws RestClientException {
     ...
     *             Reporting failure to post the file.
     */
    public <T> T postFile(final String relativeUrl, final File fileToPost, final String partName, final TypeReference<Response<T>> responseTypeReference)
            throws RestClientException {
     ...
     * @throws RestClientException .
     */
    public <T> T get(final String relativeUrl, final TypeReference<Response<T>> responseTypeReference) throws RestClientException {
     ...
     * @throws RestClientException .
     */
    public <T> T delete(final String relativeUrl, final Map<String, String> params, final TypeReference<Response<T>> responseTypeReference)
            throws RestClientException {
     ...
     * @throws RestClientException .
     */
    public <T> T delete(final String relativeUrl, final TypeReference<Response<T>> responseTypeReference) throws RestClientException {
     ...
     * @throws RestClientException .
     */
    private <T> T post(final String relativeUrl, final TypeReference<Response<T>> responseTypeReference, final HttpEntity entity) throws RestClientException {
     ...
     */
    public static String getResponseBody(final HttpResponse response) throws RestClientIOException {
    ...
    private <T> T executeRequest(final HttpRequestBase request, final TypeReference<Response<T>> responseTypeReference) throws RestClientException {
        ...
        for (int i = 0; i < numOfTrials; i++) {
            try {
                ...
                if (logger.isLoggable(Level.FINER)) {
                    logger.finer("Execute get request to " + request.getURI() + ". try number " + (i + 1) + " out of " + GET_TRIALS_NUM + ", error is "
                            + e.getMessage());
                }
                lastException = e;
            }
    ...
    private void checkForError(final HttpResponse response, final String requestUri) throws RestClientException {
        ...
        if (statusCode != HttpStatus.SC_OK) {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "[checkForError] - REST request to " + requestUri + "  failed. status code is: " + statusCode + ", response body is: "
                        + responseBody);
            }
            try {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.log(Level.FINE, "[checkForError] - REST request to " + requestUri + "  failed. throwing RestClientException: [statusCode "
                            + statusCode + " reasonPhrase " + reasonPhrase + " defaultMessage " + entity.getMessage() + " messageCode" + entity.getMessageId()
                            + "]");
                }
                throw MessagesUtils
                        .createRestClientResponseException(statusCode, reasonPhrase, entity.getVerbose(), entity.getMessage(), entity.getMessageId());
            } catch (final IOException e) {
                ...
                if (statusCode == CloudifyConstants.HTTP_STATUS_NOT_FOUND) {
                    throw MessagesUtils.createRestClientHttpException(e, statusCode, reasonPhrase, responseBody, RestClientMessageKeys.URL_NOT_FOUND.getName(),
                            requestUri);
                } else if (statusCode == CloudifyConstants.HTTP_STATUS_ACCESS_DENIED) {
                    throw MessagesUtils.createRestClientHttpException(e, statusCode, reasonPhrase, responseBody,
    ...
    private <T> T getResponseObject(final TypeReference<Response<T>> typeReference, final HttpResponse httpResponse, final String url)
        ...
        try {
            ...
            String reasonPhrase = statusLine.getReasonPhrase();
            throw MessagesUtils.createRestClientHttpException(e, statusLine.getStatusCode(), reasonPhrase, responseBody,
