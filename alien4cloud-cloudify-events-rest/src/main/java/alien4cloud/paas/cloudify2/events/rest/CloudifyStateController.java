...
public class CloudifyStateController {
    ...
    public void postEvent(@RequestParam(required = true) String application, @RequestParam(required = true) String service,
        ...
        synchronized (MUTEX) {
            ...
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine(String.format("Requesting postEvent(application=%s, service=%s, instanceId=%s, eventType=%s, event=%s)...", application, service,
                        instanceId, eventType, event));
            }
    ...
    public String deleteAllEvents(@RequestParam(required = true) String application) {
        ...
        gigaSpace.clear(template);
        BlockStorageEvent template2 = new BlockStorageEvent();
